...
# ============================
# 2. ComplexPoseAdapter (Transformer-based)
# ============================
...
def custom_collate(batch):
    poses, labels, feedbacks = zip(*batch)
    max_len = max(p.shape[0] for p in poses)
    padded_poses = [F.pad(p, (0, max_len - p.shape[0])) for p in poses]
    poses_tensor = torch.stack(padded_poses, dim=0)
    return poses_tensor, labels, feedbacks
...
if __name__ == "__main__":
    ...
    pose_adapter = ComplexPoseAdapter(input_dim=34, hidden_dim=64, output_dim=768).to(device)
